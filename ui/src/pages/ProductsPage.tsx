    ...
    return (
        <div className="min-h-screen bg-background flex flex-col font-sans transition-colors duration-300 overflow-x-hidden">
            ...
            <header className="bg-background border-b border-border px-3 sm:px-6 py-3 sm:py-4 sticky top-0 z-50">
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                    <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                        <div className="w-10 h-10 bg-amber-500/10 rounded-xl flex items-center justify-center text-amber-600 shadow-sm shrink-0 border-2 border-amber-500/20">
            ...
            <main className="max-w-7xl mx-auto w-full px-3 sm:px-6 py-6 sm:py-8 space-y-6 sm:space-y-8 animate-in fade-in duration-500 overflow-x-hidden">
                {/* Search & Stats — inline on all sizes, scan badge not below */}
                <div className="flex flex-row flex-wrap items-center gap-3 sm:gap-4">
                    <div className="relative flex-1 min-w-[200px] max-w-md flex gap-2">
                        <div className="relative flex-1 min-w-0">
                            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 w-[18px] h-[18px] pointer-events-none" />
                            <input
                                type="text"
                                placeholder="Search catalog, SKU, barcode..."
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                className="flex h-[46px] w-full rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 pl-10 pr-4 py-2 text-[14px] font-medium shadow-sm placeholder:text-slate-400 focus-visible:outline-none focus-visible:border-amber-400 focus-visible:ring-4 focus-visible:ring-amber-400/20 disabled:cursor-not-allowed disabled:opacity-50 transition-all"
                            />
                        </div>
                        <button
                            onClick={() => setShowScanner(true)}
                            className="h-[46px] px-4 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 hover:border-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/30 rounded-xl transition-all flex items-center justify-center gap-2 shadow-sm hover:shadow-md active:scale-[0.98] shrink-0"
                            title="Scan Barcode"
                        >
                            <Barcode className="w-5 h-5 text-amber-600" />
                        </button>
                    </div>
                    {/* Scan status — inline pill, not below */}
                    <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 border-2 border-emerald-500/20 rounded-full">
                            <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse shadow-sm shadow-emerald-500/50"></div>
                            <span className="text-[10px] font-black text-emerald-700 dark:text-emerald-400 uppercase tracking-widest">Scanner Ready</span>
                        </div>
                        {/* Mobile: just dot + count */}
                        <div className="sm:hidden w-8 h-8 rounded-full bg-emerald-500/10 border-2 border-emerald-500/20 flex items-center justify-center">
                            <div className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-pulse"></div>
                        </div>
                        <div className="hidden sm:block h-6 w-px bg-slate-200 dark:bg-slate-700"></div>
                        <div className="flex items-center gap-2 bg-amber-500 text-white px-3 py-1.5 rounded-full text-xs font-black shadow-md shadow-amber-500/20 border-2 border-amber-600/20">
                            <span className="hidden sm:inline">Catalog:</span> {products.length}
                        </div>
                    </div>
                </div>
