...
import { Plug, RefreshCw, Plus, Trash2, CheckCircle, XCircle } from 'lucide-react';
...
export default function IntegrationsPage() {
    ...
    const [form, setForm] = useState({ name: '', provider: '', config: '{}' });
    ...
    async function handleSync(id: string) {
        setSyncing(id);
        try {
            const res = await api.post(`/integrations/${id}/sync`, { dataType: 'products' });
            toast.success(`Synced ${res.data.results?.length || 0} items`);
        } catch (e: any) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-start justify-between">
                ...
                <button onClick={() => setShowAdd(true)} className="px-4 py-2 bg-primary text-primary-foreground rounded-xl text-xs font-black uppercase tracking-wider hover:opacity-90 transition-all flex items-center gap-2">
            ...
            {loading ? (
                ...
                <div className="text-center py-20 glass-card rounded-2xl">
                    ...
                    <button onClick={() => setShowAdd(true)} className="px-6 py-3 bg-primary text-primary-foreground rounded-xl text-xs font-black uppercase tracking-wider">
                        Add Integration
                    </button>
                </div>
            ) : (
                        ...
                        <div key={int.id} className="glass-card rounded-2xl p-6 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => handleSync(int.id)} disabled={syncing === int.id || !int.isActive}
                                    className="p-2 rounded-xl bg-secondary/30 hover:bg-secondary/50 transition-all disabled:opacity-40">
                                    <RefreshCw className={`w-4 h-4 ${syncing === int.id ? 'animate-spin' : ''}`} />
                                </button>
                                <button onClick={() => setConfirmDelete(int.id)} className="p-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-500 transition-all">
            ...
            {showAdd && (
                <div className="fixed inset-0 bg-black/60 z-[200] flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
                    <div className="glass-card rounded-2xl p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        <h2 className="text-lg font-black mb-4">Connect Service</h2>
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-3 pt-2">
                                <button onClick={() => setShowAdd(false)} className="flex-1 px-4 py-3 bg-secondary/30 rounded-xl text-xs font-black uppercase tracking-wider">Cancel</button>
                                <button onClick={handleCreate} className="flex-1 px-4 py-3 bg-primary text-primary-foreground rounded-xl text-xs font-black uppercase tracking-wider">Connect & Test</button>
                            </div>
                        </div>
                    </div>
                </div>
