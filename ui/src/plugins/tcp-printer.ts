...
export interface TcpPrinterPlugin {
    print(options: { ip: string; data: string; port?: number }): Promise<{ success: boolean; error?: string }>;
    testConnection(options: { ip: string; port?: number }): Promise<{ reachable: boolean; error?: string }>;
}

const TcpPrinter = registerPlugin<TcpPrinterPlugin>('TcpPrinter');

export async function printViaTcp(options: { ip: string; data: string; port?: number }): Promise<{ success: boolean; error?: string }> {
...
export async function testTcpConnection(ip: string, port: number = 9100): Promise<{ reachable: boolean; error?: string }> {
    if (!Capacitor.isNativePlatform()) {
